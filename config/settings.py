...
def load_config(fetch_patient_ids=False, db=None):
    ...
    # If fetch_patient_ids is True, read provider IDs from providers.json and fetch patient IDs
    if fetch_patient_ids:
        if not db:
            raise ValueError("Database instance required when fetch_patient_ids is True")
        ...
        try:
            with open("providers.json", "r") as f:
                providers_config = json.load(f)
            provider_ids = providers_config.get("provider_ids", [])
            print(f"Loaded {len(provider_ids)} provider IDs from providers.json")
        except Exception as e:
            print(f"Failed to read providers.json: {e}")
            provider_ids = []
        ...
        for provider_id in provider_ids:
            ...
            print(f"Provider {provider_id} has {len(patient_ids)} unique patients")
        ...
        settings["patient_ids"] = all_external_ids
        print(f"Fetched {len(all_external_ids)} unique external patient IDs across all providers")
    else:
        # Read config.json for patient IDs
        try:
            with open("config.json", "r") as f:
